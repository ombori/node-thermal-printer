...
class ThermalPrinter {
  ...
  cut({ verticalTabAmount = 2 } = {}) {
    ...
    if (this.printerTypes === this.types.CUSTOM_VPK) {
      this.append(Buffer.from([0x1D, 0x65, 0x03, 0x04]));
    } else {
  ...
  partialCut({ verticalTabAmount = 2 } = {}) {
    ...
    if (this.config.type === this.types.CUSTOM_VPK) {
      this.append(Buffer.from([0x1D, 0x65, 0x03, 0x04]));
    } else {
